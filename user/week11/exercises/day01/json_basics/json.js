...
const ageJson = JSON.stringify(age,undefined,1);
const isMarriedJson = JSON.stringify(isMarried,undefined,2);
const studentJson = JSON.stringify(student,undefined,2);
const jsonStudent = JSON.stringify(student,['firstName','lastName','skills'],3);
const txtObj = JSON.parse(txt,undefined,3);
const haveManySkills = JSON.parse(txt,);
const data = JSON.parse(txt);
const hwoHasManySkills = null
const maxSkillsCount = 0

for(const name in data) {
    const userSkills = data[name].skills;
    if(userSkills && userSkills.length > maxSkillsCount) {
        maxSkillsCount = userSkills.length;
        hwoHasManySkills = name
    }
}

console.log(hwoHasManySkills)
